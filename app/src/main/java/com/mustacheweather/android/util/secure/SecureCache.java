...
public class SecureCache {
    ...
    final InternalCache internalCache = new InternalCache() {
        @Override public Response get(Request request) throws IOException {
            return SecureCache.this.get(request);
        }

        @Override public CacheRequest put(Response response) throws IOException {
            return SecureCache.this.put(response);
        }

        @Override public void remove(Request request) throws IOException {
            SecureCache.this.remove(request);
        }

        @Override public void update(Response cached, Response network) {
            SecureCache.this.update(cached, network);
        }

        @Override public void trackConditionalCacheHit() {
            SecureCache.this.trackConditionalCacheHit();
        }

        @Override public void trackResponse(CacheStrategy cacheStrategy) {
            SecureCache.this.trackResponse(cacheStrategy);
        }
    };
    ...
    public SecureCache(File directory, long maxSize) {
        this(directory, maxSize, FileSystem.SYSTEM);
    }

    SecureCache(File directory, long maxSize, FileSystem fileSystem) {
    ...
    Response get(Request request) {
        ...
        try {
            entry = new SecureCache.Entry(snapshot.getSource(ENTRY_METADATA));
        } catch (IOException e) {
    ...
    private CacheRequest put(Response response) {
        ...
        SecureCache.Entry entry = new SecureCache.Entry(response);
        ...
        try {
            ...
            return new SecureCache.CacheRequestImpl(editor);
        } catch (IOException e) {
            abortQuietly(editor);
            return null;
        }
    ...
    private void update(Response cached, Response network) {
        SecureCache.Entry entry = new SecureCache.Entry(network);
        DiskLruCache.Snapshot snapshot = ((SecureCache.CacheResponseBody) cached.body()).snapshot;
        DiskLruCache.Editor editor = null;
        try {
...
//                    responseHeadersBuilder.addLenient(source.readUtf8LineStrict());
        ...
        public Response response(DiskLruCache.Snapshot snapshot) {
            ...
            return new Response.Builder()
                    ...
                    .body(new SecureCache.CacheResponseBody(snapshot, contentType, contentLength))
                    .handshake(handshake)
